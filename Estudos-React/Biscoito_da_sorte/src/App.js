...
export default class App extends Component {
  ...
  sort() {
    const numberFrase = Math.floor(Math.random() * this.frases.length);
    this.state.textFrase = ` " ${this.frases[numberFrase]} "`;
  ...
  render() {
    return (
      <div className="container">
        <img src={Photo} alt="Biscoito da sorte" className="imgSort" />
        <Button sort={this.sort} />
        <h3>{this.state.textFrase}</h3>
      </div>
    );
  }
}
